...
///////////////////////////////////////////////////////////////////// Выбор видеокарт
	...
	//cudaSetDevice используется для выбора устройства CUDA в соответствии с номером процесса, полученным из MPI_Comm_rank.
	//Это позволяет каждому процессу использовать свое собственное устройство CUDA для выполнения вычислений.
    cudaSetDevice(DEVICE);

    if (DEVICE == 0)
        std::cout << "Settings: " << "\n\tMin error: " << eps << "\n\tMax iteration: " << iter_max << "\n\tSize: " << size << "x" << size << std::endl;

	//разрешения доступа между соседними устройствами.
	//позволяет передавать данные напрямую между соседними устройствами без необходимости копирования через хост
	/*
	if (DEVICE!=0)
...
///////////////////////////////////////////////////////////////////// Выделения памяти
	...
	//выделения памяти на хосте (CPU) с использованием управляемой памяти CUDA.
	//Она позволяет выделить память, которая будет доступна и с CPU, и с GPU.
...
///////////////////////////////////////////////////////////////////// Основной цикл
	...
	while((iter < iter_max) && error > eps)	{
		...
		//MPI_Sendrecv позволяет одновременно отправлять сообщение и принимать сообщение в одном вызове функции
		//синхронный обмен данными между процессами
		//MPI_Send b MPI_Recv
